...
// ***** CUSTOM STRUCTS FOR EDRs.h ***** //

typedef struct _EDR_MAP {
    LPCWSTR pwszEDRName;
    LPCWSTR pwszProcessNames[MAX_EDR_PROCESSES];
    LPCWSTR pwszDriverNames[MAX_EDR_DRIVERS];
}EDR_MAP;

typedef enum _EDR_COMPONENT_TYPE {
    EDR_COMPONENT_PROCESS = 0,
    EDR_COMPONENT_DRIVER
} EDR_COMPONENT_TYPE;

typedef struct _EDR_HIT {
    LPCWSTR             pwszVendor;
    EDR_COMPONENT_TYPE  type;
    LPCWSTR             pwszComponent;
    ULONG               pid;
} EDR_HIT;

typedef struct _EDR_RESULTS {
    EDR_HIT hits[MAX_EDR_HITS];
    DWORD   count;
} EDR_RESULTS;

// ***** STRUCTS FOR enumProcesses.c ***** //
...
typedef struct _SYSTEM_MODULE {
    ...
    USHORT LoadCount;
    USHORT NameOffset;
    CHAR   FullPathName[256];  // NT-style path: \SystemRoot\...
} SYSTEM_MODULE, * PSYSTEM_MODULE;

typedef struct _SYSTEM_MODULE_INFORMATION {
    ULONG ModulesCount;
    SYSTEM_MODULE Modules[1]; // Or [0] if using older MSVC extensions
